...
| A1〜 | Audio | キャラクターごとの音声トラック |

字幕・音声ともにキャラクターごとに独立したトラックに配置されるため、Resolve 上でキャラクター単位の一括編集が可能。

### 音声オーバーラップ

`voices/<プロジェクト名>/metadata.json`（voicevox-parser が自動生成）が存在する場合、次のセリフの `pre_pause` が `0` のとき前のセリフのスロットを **`overlap_frames` フレーム分短縮** する。

- **字幕**: 短縮された尺でテキストクリップを作成（テキスト同士は重ならない）
- **音声**: クリップは元のフル尺を維持し、異なるキャラの音声が **`overlap_frames` フレーム重なる**
...
├── src/
│   ├── scan_project.py  # WAV ファイル走査・ファイル名解析・テキスト読み込み・metadata.json 読み込み
│   └── otio_builder.py  # OTIO クリップ・タイムライン構築（オーバーラップ処理含む）
